...
"""
function cutting_planes_length_separation_model(instance :: Data, gurobi_env)
    N = instance.N
    model_SPL = Model(() -> Gurobi.Optimizer(gurobi_env))
    set_silent(model_SPL)

    @variable(model_SPL, 0 <= delta1[i = 1:N, j = i+1:N] <= instance.delta_1_max)

    @constraint(model_SPL, 
        sum(delta1[i, j] for i in 1:N, j in i+1:N) <= instance.L
    )
...
"""
function cutting_planes_weights_separation_model(instance :: Data, gurobi_env)
    N = instance.N
    model_SPW = Model(() -> Gurobi.Optimizer(gurobi_env))
    set_silent(model_SPW)

    @variable(model_SPW, 0 <= delta2[i = 1:N] <= instance.delta_2_max[i])

    @constraint(model_SPW, 
        sum(delta2[i] for i in 1:N) <= instance.W
    )
...
"""
function separation_length(model_SPL, x, instance :: Data, timelimit :: Int = -1)
    N = instance.N
    if timelimit > 0
    ...
    @objective(model_SPL, Max,
        sum( delta1[i, j] * x[i, j] * (instance.l_hat[i] + instance.l_hat[j]) for i in 1:N, j in i+1:N )
    )
...
"""
function separation_weights(model_SPW, y, k :: Int, instance :: Data, timelimit :: Int = -1)
    N = instance.N
    if timelimit > 0
    ...
    @objective(model_SPW, Max,
        sum( (1 + delta2[i]) * y[i, k] * instance.weights[i] for i in 1:N )
    )
